...
function match(candidate, job) {
  if (!candidate.minSalary || !job.maxSalary) throw TypeError('Salary cannot be blank');
  return candidate.minSalary * 0.9 <= job.maxSalary;
}

// TESTS - run the following command to test this file:
// npm run test -- JS2-Week1/Job_Matching_1.js
...
test('Job Matching #1', () => {
  ...
  expect(match(candidate1, job2)).toBe(false);
});

test('Job Matching #1, check for error', () => {
  const candidate1 = { minSalary: 120000 };
  const job3 = { maxSalary: '' };
  try {
    match(candidate1, job3);
  } catch (e) {
